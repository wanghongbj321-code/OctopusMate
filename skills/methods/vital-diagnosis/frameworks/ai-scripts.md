...
诊断方法的 AI 引导层话术与交互规则。**遵守平台 AI 铁律**：打分规则以顾问确认的 `scoring_config` 为准（方法论锚点仅作默认参考）；AI 嵌入引导与记录，不替代顾问决策；只引用顾问确认的共识与经核验证据。
...
**打分规则确认话术**（核心，D4）：
> 现在确认本次诊断的打分规则。VITAL 方法论提供默认锚点作参考（1-5 分，0.5 步进，阻断阈值 2.0）。您可以：① 整体采用默认锚点（一键确认）；② 逐角度修改锚点文本；③ 调整步进或阻断阈值。**请确认最终打分规则，我将据此提示每个角度的打分参照。**
