---
name: vision-gate
description: 愿景质检 skill——编排出口校验与质量检验：调用引擎输出契约校验器（scripts/engine/contract.py）、复核质量检验（五项检验/六特质）、视觉审计入口（确认包 HTML 静态审计 + 浏览器视觉验证）。只编排不重复实现校验逻辑。收到「检验愿景输出」「愿景靠谱吗」等请求时使用。
---
...
1. **输出契约校验**：调用 `engine/contract.validate_output()`——平台底线核心字段必填（visionStatement / visionNarrative / ambitionTable / ambitionRationale / impactSummary）、条件必填（存在降级项时必须提供 validationPlan）。缺失核心字段 → 阻断进入确认，返回缺失清单。
2. **质量检验复核**：按方法步骤定义复核（7 步法步骤 06 五项检验 / 步骤 04 六特质自检；北极星法第 ④ 步六特质自检）——语义型判定由 AI 引导层执行，本 skill 汇总建议。
3. **未决项检查**：调用 `engine/open_issues.unowned()`——不留无主项；出口确认环节裁决（补充/降级/移出）。
