...
## 规则 4：`internal/` 目录由独立 git 仓库管理，不纳入主仓库改动

**`internal/` 由独立嵌套 git 仓库管理**（目录自身含 `.git`，非 submodule），OctopusMate 主仓库不跟踪其内容改动（主仓库 `git status` 不显示 internal 下的改动）。

- 在主仓库**新建分支 / commit / push / PR** 时，**不得纳入 `internal/` 的改动**
- `internal/` 下的改动在其**子仓库内单独提交管理**（进入 `internal/` 目录后使用独立的 git 命令操作）
- 需要修改 `internal/` 内容时，先在子仓库内提交，不要试图在主仓库中 add / commit 这些文件

> 背景约定（2026-08-20）：主仓库提交时曾反复涉及 `internal/` 改动。用户确认 `internal/` 为独立嵌套 git 仓库、由子仓库自行管理，主仓库分支与提交一律排除其改动。

## 规则 5：资产包渲染为 HTML 必须使用 LLM 生成，禁止 Python 代码

在设计与开发中，**将资产包（artifact / 交付物）渲染为 HTML 时，必须使用 LLM（大模型）直接生成 HTML 内容，不允许使用 Python 代码（脚本、模板引擎、字符串拼接等方式）来生成 HTML**。

- 适用场景：专家智能体交付物（报告、评估、方案等）需以 HTML 形式呈现时。
- 正确做法：由 LLM 直接产出完整、可读、语义化的 HTML（含内联 CSS/JS）。
- 禁止做法：用 Python（如 `open().write()`、Jinja2、f-string 拼接等）拼装或渲染 HTML 文件。
- 例外：若确有必须使用代码处理的结构化数据管线（如非展示层的预处理），须先与用户确认。

> 背景约定（2026-08-20）：用户明确要求 HTML 交付物统一由 LLM 生成，以保证内容质量与可控性，避免 Python 渲染带来的样式/结构失控。
